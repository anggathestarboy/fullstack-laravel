        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm text-left">
                ...
                <tbody>
                    ...
                    <tr class="border-t cursor-pointer" onclick="document.getElementById('publisherDetailModal-{{ $publisher->publisher_id }}').showModal()">
                        ...
                        <div class="modal-box bg-white rounded-lg shadow-lg w-full max-w-xl fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style="padding: 20px">
                            ...
                            <form action="{{ route('admin.publisher.update', ['publisher_id' => $publisher->publisher_id]) }}" method="POST" class="space-y-4">
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    <button type="submit" class="px-3 py-2 bg-gray-800 rounded text-sm text-white font-medium transition-all duration-300">Update</button>
                                </div>
                            </form>
                            ...
                            <form action="{{ route('admin.publisher.delete', ['publisher_id' => $publisher->publisher_id]) }}" method="POST" class="mt-2 flex justify-end">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-3 py-2 bg-red-600 rounded text-sm text-white font-medium transition-all duration-300">Delete</button>
                            </form>
                        </div>
                    </dialog>
                    @endforeach
